...
.PHONY: setup
setup:
	cd src/python; python3 -m venv venv; source venv/bin/activate; pip install -r requirements.txt ; DJANGO_SETTINGS_MODULE=settings python manage.py makemigrations ; DJANGO_SETTINGS_MODULE=settings python manage.py migrate
